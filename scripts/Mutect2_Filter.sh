...
helpFunction()
{
  ...
  echo ""
  echo "Usage: ./ShainMutectFilter/scripts/Mutect2_Filter.sh Tumor.funcotator.txt Tumor.bam Normal.bam genome.fa (male/female)"
  echo ""
}
...
if [[ (${1: -4} != ".txt") || (${2: -4} != ".bam") || (${3: -4} != ".bam") || (${4: -3} != ".fa" && ${4: -6} != ".fasta") || (${5: -4} != "male" && ${5: -4} != "Male") ]]
	then
		...
		echo ""
		echo "e.g. ./ShainMutectFilter/scripts/Mutect2_Filter.sh Tumor.funcotator.txt Tumor.bam Normal.bam genome.fa (male/female)"
		exit
fi
